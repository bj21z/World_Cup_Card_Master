...
(function(){
 ...
 function starters(team,save){ if(window.Lineup&&save&&save.lineups&&save.lineups[team])return Lineup.players(team,save,'starters'); return roster(team,save).slice(0,11); }
 ...
 function train(id,save,focus=null){ if((save.coins||0)<BALANCE.trainCost) return {ok:false,msg:'金币不足，请通过比赛获得金币。'}; save.coins-=BALANCE.trainCost; let idx=focus===null?Math.floor(Math.random()*6):Number(focus); save.boost[id]=save.boost[id]||[0,0,0,0,0,0]; save.boost[id][idx]=Math.min(15,(save.boost[id][idx]||0)+1+Math.floor(Math.random()*2)); save.xp[id]=(save.xp[id]||0)+22; return {ok:true,idx,stat:STAT_NAMES[idx]}; }
